...
class Otodom:
    def __init__(self):
        self.LISTING_INFORMATION_RETRIEVAL_FUNCTIONS = [
    ...
    def otodom_scrap(self, base_search_url, page_limit=1):
        dataframe = pd.DataFrame()
        generator = self.otodom_url_generator(base_search_url)
    ...
    def otodom_url_generator(self, url):
    ...
    def get_data_from_otodom_listing(self, listing):
        ...
        for listing_extractor in self.LISTING_INFORMATION_RETRIEVAL_FUNCTIONS:
            data = pd.Series(listing_extractor(listing))
            listing_data = listing_data.append(data)

        return listing_data

    def append_new_listing_data(self, dataframe: pd.DataFrame,
    ...
    def get_price(self, soup: BeautifulSoup) -> Dict[str, Optional[int]]:
        ...
        price = int(
            re.sub(
                pattern=r"[^0-9,.]", repl="", string=price_div[0],
                flags=re.UNICODE
            )
        )
        return {"price": price}
    ...
    def get_building_type(self, soup: BeautifulSoup) -> Dict[
        str, Optional[str]]:
        soup_filter = {"aria-label": "Rodzaj zabudowy"}

        type_of_building_div = _extract_divs(soup, soup_filter,
                                             "type-of-building")
    ...
    def get_year_of_construction(self, soup: BeautifulSoup) -> Dict[
        str, Optional[int]]:
    ...
    def get_number_of_rooms(self, soup: BeautifulSoup) -> Dict[
        str, Optional[int]]:
        soup_filter = {"aria-label": "Liczba pokoi"}

        number_of_rooms_div = _extract_divs(soup, soup_filter,
                                            "number_of_rooms")
    ...
    def get_ownership_form(self, soup: BeautifulSoup) -> Dict[
        str, Optional[str]]:
    ...
    def get_construction_material(self, soup: BeautifulSoup) -> Dict[
        str, Optional[str]]:
        soup_filter = {"aria-label": "Materiał budynku"}

        material_div = _extract_divs(soup, soup_filter,
